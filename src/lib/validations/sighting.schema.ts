...
export const sightingSchema = z.object({
  lost_report_id: z.string({
    errorMap: () => ({ message: 'Identificador de reporte de mascota inválido' }),
  }),
  latitude: z.number().min(-90).max(90),
  longitude: z.number().min(-180).max(180),
  approximate_address: z.string({
    errorMap: () => ({ message: 'Por favor indicá la esquina o calle donde viste a la mascota' }),
  }).min(3, 'Por favor indicá la esquina o calle donde viste a la mascota').max(150),
  sighting_date: z.string({
    errorMap: () => ({ message: 'Por favor indicá cuándo la viste' }),
  }).min(1, 'Por favor indicá cuándo la viste'),
  photo_url: z.string().optional().nullable(),
  description: z.string({
    errorMap: () => ({ message: 'Por favor describí hacia dónde iba o qué viste (mínimo 5 caracteres)' }),
