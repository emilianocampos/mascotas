...
export const lostReportSchema = z.object({
  // Pet information
  name: z.string({
    errorMap: () => ({ message: 'Por favor ingresá el nombre de la mascota' }),
  ...
  size: z.enum(['small', 'medium', 'large', 'giant'], {
    errorMap: () => ({ message: 'Por favor seleccioná el tamaño aproximado de la mascota' }),
  }),
  primary_color: z.string({
    errorMap: () => ({ message: 'Por favor indicá el color principal (ej. Negro, Dorado, Marrón)' }),
  ...
  approximate_address: z.string({
    errorMap: () => ({ message: 'Por favor indicá el barrio, plaza o calle donde se extravió' }),
  }).min(3, 'Por favor indicá el barrio, plaza o calle donde se extravió').max(150),
  last_seen_date: z.string({
    errorMap: () => ({ message: 'Por favor indicá la fecha y hora aproximada de extravío' }),
  }).min(1, 'Por favor indicá la fecha y hora aproximada de extravío'),
  city_id: z.number().int().positive().optional().nullable(),

  // Description & Contact
  description: z.string({
    errorMap: () => ({ message: 'Por favor escribí cómo se extravió o qué collar llevaba (mínimo 10 caracteres)' }),
  }).min(10, 'Por favor escribí cómo se extravió o qué collar llevaba (mínimo 10 caracteres)').max(1000),
  contact_name: z.string({
    errorMap: () => ({ message: 'Por favor ingresá tu nombre o apodo de contacto' }),
  }).min(2, 'Por favor ingresá tu nombre o apodo de contacto').max(70),
  contact_phone: z.string({
    errorMap: () => ({ message: 'Por favor ingresá un teléfono o WhatsApp para que te avisen' }),
