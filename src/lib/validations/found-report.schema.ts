...
export const foundReportSchema = z.object({
  ...
  species: z.enum(['dog', 'cat', 'bird', 'rabbit', 'other'], {
    errorMap: () => ({ message: 'Por favor seleccioná qué tipo de animal es (perro, gato, etc.)' }),
  }),
  breed: z.string().max(80, 'Raza demasiado larga').optional().nullable(),
  gender: z.enum(['male', 'female', 'unknown']).default('unknown'),
  size: z.enum(['small', 'medium', 'large', 'giant'], {
    errorMap: () => ({ message: 'Por favor seleccioná el tamaño aproximado del animal' }),
  }),
  primary_color: z.string({
    errorMap: () => ({ message: 'Por favor indicá el color principal (ej. Blanco, Marrón, Negro)' }),
  ...
  approximate_address: z.string({
    errorMap: () => ({ message: 'Por favor indicá la calle, esquina o barrio donde lo viste' }),
  }).min(3, 'Por favor indicá la calle, esquina o barrio donde lo viste').max(150),
  found_date: z.string({
    errorMap: () => ({ message: 'Por favor indicá la fecha y hora aproximada del hallazgo' }),
  }).min(1, 'Por favor indicá la fecha y hora aproximada del hallazgo'),
  is_holding: z.boolean({
    errorMap: () => ({ message: 'Indicá si lo tenés en tu casa o si quedó en la vía pública' }),
  }),
  city_id: z.number().int().positive().optional().nullable(),

  // Description & Contact
  description: z.string({
    errorMap: () => ({ message: 'Por favor escribí una breve descripción del estado del animal (mínimo 10 caracteres)' }),
  }).min(10, 'Por favor escribí una breve descripción del estado del animal (mínimo 10 caracteres)').max(1000),
  contact_name: z.string({
    errorMap: () => ({ message: 'Por favor ingresá tu nombre para que el dueño sepa con quién habla' }),
  }).min(2, 'Por favor ingresá tu nombre para que el dueño sepa con quién habla').max(70),
  contact_phone: z.string({
    errorMap: () => ({ message: 'Por favor ingresá un teléfono o WhatsApp de contacto' }),
